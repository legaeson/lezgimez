...
async function main() {
    ...
    const archiveName = `versions\\ver_${newVersion}.zip`;
    ...
    try {
        // Build PowerShell command to exclude versions, 1Vers, node_modules, .git, and make_backup.bat
        const psCommand = `powershell.exe -Command "$items = Get-ChildItem -Path . | Where-Object { $_.Name -ne 'versions' -and $_.Name -ne '1Vers' -and $_.Name -ne 'node_modules' -and $_.Name -ne '.git' -and $_.Name -ne 'make_backup.bat' -and $_.Name -notmatch '\\.zip$' }; Compress-Archive -Path $items.FullName -DestinationPath '${archiveFullPath}' -Force"`;
